#!/usr/bin/env python3
"""
BABY STEP 4: Live LLM + Mock Tool Execution
...
def test_step4():
    print("="*60)
    print("BABY STEP 4: Live LLM + Mock Tool Execution")
    print("="*60)
    ...
    try:
        ...
        # 2. Setup Prompt Builder with Tools
        from doit.core.prompt_builder import SingleLinePromptBuilder
        builder = SingleLinePromptBuilder()
        builder.register_tool("file_read", "Reads content from a local file path", {"path": {"type": "string"}})
        builder.register_tool("bash_execute", "Runs a shell command and returns stdout", {"command": {"type": "string"}})
        builder.register_tool("file_write", "Writes content to a local file", {"path": {"type": "string"}, "content": {"type": "string"}})
        print("✅ Registered 3 tools in PromptBuilder")

        # 3. Create Mock Dispatcher
