#!/usr/bin/env python3
"""STEP 7: Real Browser Ops Integration Test (Real LLM + Real Edge)"""
import sys
import asyncio
import argparse
import http.server
...
PROJECT = "step7-browser-test"

TEST_HTML = """
<!DOCTYPE html>
<html lang="en"><body>
  <h1 id="title">Step 7 Browser Ops Test</h1>
  <button id="action-btn">Click Me</button>
  <p id="status">Ready for LLM</p>
  <script>
    document.getElementById('action-btn').addEventListener('click', () => {
      document.getElementById('status').textContent = 'Button Activated by Browser Ops';
    });
  </script>
</body></html>
"""
...
def test_step7():
    print("="*60)
    print("STEP 7: REAL BROWSER OPS (Real LLM + Real Edge)")
    print("="*60)
    logger.info("Step 7 test started | debug_mode=%s", debug_mode)
    ...
    try:
        ...
        goal = (
            ...
            f"wait for '#action-btn' to be visible, "
            f"click the exact text 'Click Me', "
            f"and write 'Browser Ops Executed Successfully' to 'output/action_log.txt'."
        )
        print(f"\n🎯 Goal: {goal}")
        ...
        if output_file.exists():
            ...
            if "Browser Ops Executed Successfully" in content:
                print("✅ VERIFIED: Real LLM chained browser tools + file_write")
                logger.info("Step 7 integration passed")
            else:
        ...
        print(f"\n❌ Test Failed: {e}")
        logger.error("Step 7 test failed: %s", e, exc_info=False)
        import traceback; traceback.print_exc()
    finally:
        try:
            os.chdir(original_cwd)
            httpd.server_close()
            logger.info("Local server socket closed")
        except Exception as e:
        ...
        logger.info("Step 7 test finished")

if __name__ == "__main__":
    ctx = init_session_logger(WORKSPACE)
    logger.info("Starting Step 7 Browser Ops Test")
    test_step7()