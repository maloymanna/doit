# src/doit/core/state_manager.py [MOD v1.5]
import sqlite3
import json
import hashlib
...
class SQLiteStateManager:
    def __init__(self, workspace_dir: Path):
        ...
        self.db_path.parent.mkdir(parents=True, exist_ok=True)  # ✅ Guarantees dir exists
        self._init_db()

    def _init_db(self):
        # Inline schema to eliminate fragile repo-root path resolution
        schema = """
        CREATE TABLE IF NOT EXISTS goals (
            goal_id TEXT PRIMARY KEY,
            project TEXT,
            query TEXT,
            status TEXT DEFAULT 'planning' CHECK(status IN ('planning', 'in_progress', 'completed', 'blocked', 'failed', 'requires_user_confirmation')),
            plan_json TEXT,
    ...
    def create_goal(self, goal_id: str, project: str, query: str, max_steps: int = 15) -> str:
        self._exec_and_close("INSERT OR IGNORE INTO goals (goal_id, project, query, max_steps) VALUES (?, ?, ?, ?)", (goal_id, project, query, max_steps))
        return goal_id
    ...
    def log_step(self, goal_id: str, step_index: int, tool_name: str, params: dict, result: str, rationale: str, status: str = "completed"):
        conn = self._get_conn()
        try:
            conn.execute("INSERT INTO steps (goal_id, step_index, tool_name, parameters, result, rationale, status) VALUES (?, ?, ?, ?, ?, ?, ?)",
    ...
    def log_audit(self, goal_id: str, event_type: str, prompt_snippet: str, response_snippet: str):
        ...
        self._exec_and_close("INSERT INTO audit_log (goal_id, event_type, prompt_hash, response_snippet) VALUES (?, ?, ?, ?)",
    ...
    def get_recent_context(self, goal_id: str, limit: int = 5) -> List[Dict[str, Any]]:
        rows = self._exec_and_close("SELECT step_index, tool_name, parameters, result, rationale, status FROM steps WHERE goal_id = ? ORDER BY step_index DESC LIMIT ?", (goal_id, limit), fetch=True)
